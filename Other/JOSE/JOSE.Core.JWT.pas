...
type
  ...
  private
    ...
    function GetCertArray(): TArray<string>;
    procedure SetCertArray(const Values: TArray<string>);
  public
    ...
    property CertArray: TArray<string> read GetCertArray write SetCertArray;
  end;
...
function TJWTHeader.GetCertArray: TArray<string>;
var
  JsonCertArray: TJSONArray;
  I: Integer;
begin
  SetLength(Result, 0);

  JsonCertArray := TJSONObject.ParseJSONValue(TJSONUtils.GetJSONValue(THeaderNames.CERT_ARRAY, FJSON).AsString) as TJSONArray;
  if not Assigned(JsonCertArray) then
    Exit;

  SetLength(Result, JsonCertArray.Count);
  for I := 0 to JsonCertArray.Count - 1 do
    Result[I] := JsonCertArray.Items[I].GetValue<String>;
end;
...
procedure TJWTHeader.SetCertArray(const Values: TArray<string>);
var
  JsonCertArray: TJSONArray;
  Cert: String;
begin
  ...
  JsonCertArray := TJSONArray.Create();
  for Cert in Values do
    JsonCertArray.Add(Cert);
  FJSON.AddPair(THeaderNames.CERT_ARRAY, JsonCertArray);
end;
